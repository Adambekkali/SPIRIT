...
import { Epreuve, Competition, Participer, Couple } from "@prisma/client";

type ParticiperWithCouple = Participer & { couple: Couple };

type EpreuveWithCompetition = Epreuve & {
  competition: Competition;
  participations: ParticiperWithCouple[];
};
...
const Epreuves = () => {
  const [epreuves, setEpreuves] = useState<EpreuveWithCompetition[]>([]);
  const [selectedEpreuve, setSelectedEpreuve] =
    useState<EpreuveWithCompetition | null>(null);
  ...
  if (loading)
    return (
      <p className="text-center text-gray-500">Chargement des épreuves...</p>
    );
      ...
      {selectedEpreuve && (
        <div>
          <h2 className="text-xl font-semibold mb-2">
            {selectedEpreuve.intitule}
          </h2>
          <p className="text-gray-700">
            <strong>Compétition :</strong>{" "}
            {selectedEpreuve.competition?.intitule}
          </p>
          <p className="text-gray-700">
            <strong>Type :</strong> {selectedEpreuve.competition?.type}
          </p>
          <p className="text-gray-700">
            <strong>Statut :</strong> {selectedEpreuve.statut}
          </p>

          {/* Liste des couples engagés */}
          <h3 className="text-lg font-semibold mt-4">
            Ordre de passage et résultats
          </h3>
          <table className="w-full border-collapse border border-gray-300 mt-2">
            ...
            <tbody>
              {selectedEpreuve.participations.map(
                (part: ParticiperWithCouple, index: number) => (
                  <tr key={index} className="border-t">
                    <Td>{index + 1}</Td>
                    <Td>{part.couple.numero_licence}</Td>
                    <Td>
                      {part.couple.nom_cavalier} {part.couple.prenom_cavalier}
                    </Td>
                    <Td>{part.couple.nom_cheval}</Td>
                    <Td>{part.couple.ecurie}</Td>
                    <Td>{part.couple.coach}</Td>
                    <Td>{part.numero_passage}</Td>
                    <Td>{part.temps ? `${part.temps} sec` : "N/A"}</Td>
                    <Td>{part.penalite ? `${part.penalite} sec` : "N/A"}</Td>
                    <Td>
                      {part.temps_total ? `${part.temps_total} sec` : "N/A"}
                    </Td>
                    <Td>{part.classement}</Td>
                    <Td>
                      <StatusBadge status={part.statut} />{" "}
                      {/* Use statut from Participer */}
                    </Td>
                    <Td>
                      <button
                        ...
                        onClick={() => {
                          const queryParams = new URLSearchParams({
                            participation_id: part.id.toString(),
                          }).toString();
...
const Th = ({ children }: { children: React.ReactNode }) => (
  <th className="border-2 border-gray-300 px-4 py-2 text-left font-medium">
    {children}
  </th>
);
